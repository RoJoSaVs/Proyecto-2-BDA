...
def get_query_1(ong):
	query1 = "MATCH (o:ONG) -[d:develops]-> (p:Project) WHERE o.o_name = $query_Value RETURN p.p_name, p.p_country"
...
def get_query_2(project):
	query2 = "MATCH  (v:Volunteer)- [w:works_on] -> (p:Project) WHERE p.p_name = $query_Value RETURN v.v_name, p.p_name, p.p_country, p.p_reach, p.p_duration"
...
def get_query_3():
	query3 = "MATCH (v:Volunteer)-[w:works_on]->(p:Project) RETURN p.p_name, p.p_country, count(v)"
...
def get_query_4(volunteer):
	query4 = "MATCH (v:Volunteer)-[w:works_on]->(p:Project) WHERE v.v_name = $query_Value RETURN p.p_name, p.p_country, p.p_reach, v.v_name"
...
def get_query_5():
	query5 = "MATCH (p:Project) RETURN p.p_reach, count(p.p_reach)"
